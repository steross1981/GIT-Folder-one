...
public class ItemScanner {
	...
	/*
	 * This code was called repeatedly so I made it a method to add Items to an
	 * arraylist via user input of values Takes parameters of Scanner to input
	 * values and Arraylist to store the values
	 */
	...
	public ArrayList<Item> scanItems() {
		...
		// calls the add items method to prompt the user for input and add character to
		// array
		addItems(itemCodes);

		// while loop checks that a boolean is false then loops until it is true;
		do {
			// switch evaluates input value for code then for each case that evaluates true
			// it adds the character to the array itemCodes
			switch (code) {
			case 'a':
				addItems(itemCodes);
				break;
			...
			case 'x':
				...
				break;
			default:
				// if the user inputs in a wrong value this case prompts them for a correct
				// value
